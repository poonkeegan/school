...
\begin{enumerate}
    ...
    \item In lecture we proved that the roots of the Chebyshev polynomial
        ...
        \begin{enumerate}
            ...
            Since the Chebyshev polynomial is infact a polynomial, with coefficient $2^{k-1}$, we can write it out as $2^{k-1}(x-x_1)(x-x_2)\cdots(x-x_k)$, where $x_i,\, 1\leq i \leq k$ are the roots of $x_i$. It can also be converted to a monic polyomial by dividing by $2^{k-1}$. To show that they are optimal, we prove the following:
            \begin{theorem}
                If W(x) is a monic polynomial of degree $\leq k$, then $\displaystyle \max_{x} |W(x)| \geq \max_{x} \Big| \frac{T_k(x)}{2^{k-1}}\Big|$
            \end{theorem}
            \begin{proof}
                Suppose $\max_{x} |W(x)| < \max_{x} \Big| \frac{T_k(x)}{2^{k-1}}\Big|$, then in particular $|W(y_i) < \frac{T_k(y_i)}{2^{k-1}}\Big|$
            \end{proof}
        \end{enumerate}
    \newpage
