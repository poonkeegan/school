...
\begin{enumerate}
    ...
    As part of your solution, provide a sketch showing the appropriate orientations. (For this question you may draw the sketch by hand or use symbolic algebra software.)
        ...
        The boundary curve of the plane is the circle at $z = 0$ with radius $2$. Since the normal vector is downward pointing, the curve is parametrized counter clockwise. So parametrize the curve as $\boldsymbol \gamma(\theta) = (2\cos \theta, 2\sin \theta, 0)$
        \begin{align*}
            &\int_{\boldsymbol \gamma} (2(2\sin\theta) - 0)(-2\sin \theta) + ((2\cos \theta) + (2\sin \theta)^2 - 0)(2\cos \theta) \, d\theta \\
            &= \int_{\boldsymbol \gamma} -8 \sin ^2 \theta + 4 \cos ^2 \theta + 8 \sin^2 \theta \cos \theta
        \end{align*}

    \newpage
    \item Let $\omega = yz\, dx - xz \, dy + xy \, dz$ and let $\boldsymbol \gamma(t) = (2\cos t, 2\sin t, 4),\, 0 \leq t \leq 2\pi$.
    \begin{enumerate}
        \item Let $S$ be the piece of the surface $z = x^2 + y^2$ with $z \leq 4$. Use Stokes' theorem to give an integral over $S$ which is equivalent to $\displaystyle \int_0^{2\pi} \omega$. Verify by directly computing both integrals.
        \item Let $S'$ be the part of the plane $z=4$ with $x^2 + y^2 \leq 4$. Use Stokes' theorem to give an integral over $S'$ which is equivalent to $\displaystyle \int_{2\pi} \omega$. Verify by direct computation.
