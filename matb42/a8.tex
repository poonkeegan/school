...
\begin{enumerate}
    ...
    \item Let $S$ be the surface defined by $x^2 + y^2 = 1$ for $0 \leq z \leq 1$ and by $x^2 + y^2 = z^2$ for $1 \leq z \leq 2$.
        \begin{enumerate}
            ...
            Examining the rightmost point (If projected into the $xy$-plane), where $\theta = 0$, both vectors will point towards the left  as $-\cos(0) = -1$ (since $u > 0$). These normals are orientation reversing, so their integrals will need to be of the opposite sign.
                ...
                \[\int_S \boldsymbol F \, d \boldsymbol S = \int_{\boldsymbol \Phi_1} F(\boldsymbol \Phi_1) \, d \boldsymbol S + \int_{\boldsymbol \Phi_2} F(\boldsymbol \Phi_2) \, d \boldsymbol S \]
            \begin{align*}
                \int_{\boldsymbol \Phi_1} F(\boldsymbol \Phi_1) \, d \boldsymbol S &= - \int_0^1 \int_0^{2\pi} -(\sin\theta)(-\cos\theta) + (\cos\theta)(- \sin \theta) + (u)(0) \, d\theta \, du = 0 \\
                \int_{\boldsymbol \Phi_2} F(\boldsymbol \Phi_2) \, d \boldsymbol S &= - \int_0^1 \int_0^{2\pi} -(u\sin\theta)(-u\cos\theta) + (u\cos\theta)(- u\sin \theta) + (u)(u) \, d\theta \, du  \\
    ...
    \item Evaluate the (vector) surface integral $\displaystyle \int_S \boldsymbol F \cdot \, d \boldsymbol S$ in each of the following cases.
        \begin{enumerate}
            ...
            \end{align*}
            Since $\sin\varphi, \cos\varphi \geq 0$ for $\varphi \in [0,\pi/2]$ this normal is orientation reversing, the sign of the integral will need to be flipped.
