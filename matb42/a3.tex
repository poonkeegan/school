...
\begin{enumerate}
    \item the straight line segment in $\mathbb{R}$ from (1,2,3) to (4,-5,6).
        ...
        We want $\boldsymbol{\gamma}_2$ to range over 2 to -5, so we can take $\boldsymbol{\gamma}_1(t)= -7t+2$ over $0\leq t \leq 3$

        We want $\boldsymbol{\gamma}_3$ to range over 3 to 6, so we can take $\boldsymbol{\gamma}_1(t)= t+3$ over $0\leq t \leq 3$
        \[ \boldsymbol{\gamma}(t) = \Big(t+1,2-7t, t+3 \Big),\: 0 \leq t \leq 3 \]
