    ...
    \item (For this question assume that all curves are of class $C^k$, some $k \geq 3$).
        ...
        \begin{enumerate}
            \item If $\gam : [a,b] \rightarrow \mathbb{R}^3$ is a regular curve, show that $\boldsymbol{T}'(t) \cdot \boldsymbol{T}(t) = 0$.
                ...
                \begin{align*}
                    \norm{\boldsymbol{T}(t)}^2 &= T_1^2 + T_2^2 + T_3^2 = 1\\
                    \deriv{t} \norm{\boldsymbol{T}(t)}^2 &= 2 T_1T'_1 + 2T_2T'_2 + T_3T'_3 = \deriv{t}1\\
                    2 (\boldsymbol{T}'(t) \cdot \boldsymbol{T}(t)) &= 2(T'_1T_1 + T'_2 T_2 + T'_3 T_3) = 0
                \end{align*}
        \end{enumerate}
        ...
        \begin{enumerate}[resume] 
            \item Show the following about the $\boldsymbol{T}, \boldsymbol{N}$ and $\boldsymbol{B}$ system
                ...
                \begin{enumerate}[label=(\roman*)]
                    \item $\deriv{t} \boldsymbol{B} \cdot \boldsymbol{B} = \deriv{t}\norm{\boldsymbol{B}}^2$, but the norm of $\boldsymbol{N}$ and $\boldsymbol{T}$ are 1, so $\norm{\boldsymbol{B}}^2 = 1$ which means that $\deriv{t} 1 = 0$
                    \item Because $\boldsymbol{B}$ is the cross product of $\boldsymbol{T}$ along with $\boldsymbol{N}$, $\boldsymbol{B} \cdot \boldsymbol {T}$ must be 0, so $\deriv{t}0 = 0$
                    \item From the last two parts, $\deriv{t}\boldsymbol{B}$ is orthogonal to both $\boldsymbol{B}$ and $\boldsymbol{T}$. Since there are only 3 dimensions for $\mathbb{R}^3$, anything orthgonal to both of these must be parallel to each other. We know that $\boldsymbol{N}$ is orthgonal to $\boldsymbol{T}$ by (a), and by definition it is orthgonal to $\boldsymbol{B}$ so both are parallel, i.e. they are scalar multiples.
                \end{enumerate}

        \end{enumerate}
        If $\gam(s)$ is a unit speed curve we can define the \textit{tortion} $\tau$ by $\frac{d \boldsymbol{B}}{ds} = - \tau \boldsymbol{N}$.
        \begin{enumerate}[resume] 
            \item Compute the torsion of $\boldsymbol{\sigma}(t) = \frac{1}{\sqrt{2}}(\cos t, \sin t, t)$.

                (see page 235, \#21(c))

                \begin{align*}
                    \boldsymbol{T}(t) &= \frac{1}{\sqrt{2}}(- \sin t, \cos t, 1) \\
                    \boldsymbol{N}(t) &= (- \cos t, - \sin t, 0) \\
                    \boldsymbol{B}(t) &= (0 + \sin t, -\cos t - 0, \sin^2 t + \cos^2 t) \\
                    \boldsymbol{B}(t) &= (\sin t, - \cos t, 1) \\
                    \deriv{t} \boldsymbol{B}(t) &= (\cos t,  \sin t, 0) = - \boldsymbol{N} \implies \tau = 1 \\
                \end{align*}
        \end{enumerate}
    \newpage
        ...
        \begin{enumerate}
            ...
            \item Evaluate $\displaystyle \int_{\boldsymbol{c}} \boldsymbol{F} \cdot d \boldsymbol{s}$, where $\boldsymbol{F}(x,y,z) = (2xy,4y,x^2)$.
                \begin{align*}
                    ...
                    &= \int^{0}_0 16 u^2 (1 - u^2) - 16 u^4 + 8u \ du - 32\int_0^{2\pi} \cos^2 t (1 - \cos^2 t)\sin t \ dt \\
                    &= - 32\int_0^{2\pi} \cos^2 t (1 - \cos^2 t)\sin t \ dt \\
                    &\text{Let }u = \cos t, \ du = -\sin t \\
                    &=  32\int_1^{1} u^2 (1 - u^2 ) \ du \\
                    &= 0
                \end{align*}
        \end{enumerate}
\end{enumerate}
